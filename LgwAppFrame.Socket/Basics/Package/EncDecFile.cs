...
namespace LgwAppFrame.SocketHelper.Basics.Package
{
    ...
    internal class EncDecFile
    {
        ...
        internal static byte[] FileSubjectDecrypt(FileState fileSend, byte[] receiveDate)
        {
            byte[] haveDate = null;
            if (fileSend.StateFile == 2)//说明我这里已经暂停了；要发一个暂停的消息给对方
                return FileSevenEncryption(CipherCode._receiveUser, CipherCode._sendStop, fileSend.FileLabel);
            ...
            catch { return EncDecFile.FileSevenEncryption(CipherCode._receiveUser, CipherCode._fileCancel, fileSend.FileLabel); }
            fileSend.FileOkLenth = fileSend.FileOkLenth + haveDate.Length;
            haveDate = FileSevenEncryption(CipherCode._receiveUser, CipherCode._dateSuccess, fileSend.FileLabel);
            return haveDate;
        }
        /// <summary>
        /// 对7位数的普通通信进行加密；
