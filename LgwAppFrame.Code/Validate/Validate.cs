...
namespace LgwAppFrame.Code
{
    public class Validate
    {
        ...
        private static Regex RegIp = new Regex(@"(?=(\b|\D))(((\d{1,2})|(1\d{1,2})|(2[0-4]\d)|(25[0-5]))\.){3}((\d{1,2})|(1\d{1,2})|(2[0-4]\d)|(25[0-5]))(?=(\b|\D))");
        private static Regex RegMac = new Regex(@"^[A-F0-9]{2}(-[A-F0-9]{2}){5}$");
        ...
        /// 检测是否为MAC
        /// </summary>
        /// <param name="inputMac">MAC地址"-"</param>
        /// <returns></returns>
        public static bool IsMAC(string inputMac)
        {
            Match m = RegMac.Match(inputMac);
            return m.Success;
        }
        /// <summary>
        /// 检测是否为IP
        /// </summary>
        /// <param name="inputIP">IP地址</param>
        /// <returns></returns>
        public static bool IsIP(string inputIP)
        {
            Match m = RegIp.Match(inputIP);
            return m.Success;
        }
        #endregion
     ...
     /// 取得网页标签内的内容
     /// </summary>
     /// <param name="httpContent">网页内容</param>
     /// <param name="httpTag">html标签</param>
     /// <returns></returns>
        public static string httpTagContent (string httpContent ,string httpTag)
        {
            Regex RegHttpTagContent = new Regex("(?<=("+ httpTag + "=\"))(.*?)(?=(\">))");//正则出 tkk:'与 ',之间的文字

            return RegHttpTagContent.Match(httpContent).ToString();
        }
        #endregion
